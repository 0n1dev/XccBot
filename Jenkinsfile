pipeline {
    ...
    environment {
        ...
        envDiscordToken = "${env.DISCORD_TOKEN}"
    ...
    stages {
        ...
        stage('Test') {
            steps {
                ...
                sh 'chmod +x gradlew'      // 모든 사용자에게 gradlew 권한 추가
                sh './gradlew clean test -Djda.token=${envDiscordToken}' // gradlew 의 Test 를 실행
        ...
        stage('SSH transfer') {
            steps([$class: 'BapSshPromotionPublisherPlugin']) {
                sshPublisher(
                        ...
                        publishers: [
                                sshPublisherDesc(
                                        configName: "XccBot",//Jenkins 시스템 정보에 사전 입력한 서버 ID
                                        verbose: true,
                                        transfers: [
                                                sshTransfer(
                                                        ...
                                                        execCommand: "nohub sh /home/xcc_bot_01/restart.sh &" //원격지에서 실행할 커맨드
                                                )
                                        ]
                                )
                        ]
